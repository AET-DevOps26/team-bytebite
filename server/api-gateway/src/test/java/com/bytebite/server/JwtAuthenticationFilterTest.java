...
class JwtAuthenticationFilterTest {
    ...
    void protectedApiRejectsMissingBearerToken() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/grocery-list").build());
    ...
    void protectedApiInjectsUserHeadersFromValidJwtAndOverwritesClientValues() {
        ...
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/grocery-list")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + token(userId, "ada@example.com", 3600))
        ...
        assertThat(chain.exchange().getRequest().getHeaders().getFirst("X-User-Email")).isEqualTo("ada@example.com");
    }
    ...
    void expiredJwtIsRejected() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/grocery-list")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + token(UUID.randomUUID(), "ada@example.com", -1))
                        .build());
    ...
    private static String token(UUID userId, String email, long expiresInSeconds) {
        ...
        String payload = "{\"sub\":\"" + userId + "\",\"email\":\"" + email + "\",\"exp\":" + (now + expiresInSeconds) + "}";
