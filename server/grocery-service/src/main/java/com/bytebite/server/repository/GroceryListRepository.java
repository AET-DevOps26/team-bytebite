...
public interface GroceryListRepository extends JpaRepository<GroceryList, UUID> {
    ...
    /**
     * Returns each list as a summary with item totals computed in a single aggregate query,
     * so the collapsed cards can show progress without loading every item.
     */
    @Query("""
