...
public class RecipeService {
  ...
  public List<RecipeSummaryDTO> getAll(UUID userId) {
    return repository.findAllByUserIdOrderByCreatedAtDesc(userId).stream()
        .map(
            recipe -> new RecipeSummaryDTO(recipe.getId(), recipe.getName(), recipe.getCreatedAt()))
        .toList();
  }
  ...
  private Recipe requireOwned(UUID id, UUID userId) {
    return repository
        ...
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Recipe not found: " + id));
  }

  private RecipeDetailDTO toDetail(Recipe recipe) {
    List<RecipeItemResponseDTO> items =
        recipe.getItems().stream()
            .map(
                item ->
                    new RecipeItemResponseDTO(
                        item.getId(),
                        item.getName(),
                        item.getQuantity(),
                        item.getUnit(),
