...
public class GroceryListService {
  ...
  @Transactional
  public GroceryItemResponseDTO setItemPurchased(
      UUID id, UUID itemId, UUID userId, boolean purchased) {
    GroceryList groceryList = requireOwned(id, userId);
    GroceryItem item =
  ...
  private List<GroceryItem> buildItems(
      List<GroceryItemRequestDTO> itemDtos, GroceryList groceryList) {
  ...
  private GroceryList requireOwned(UUID id, UUID userId) {
    return repository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Grocery list not found: " + id));
  }

  private GroceryListDetailDTO toDetail(GroceryList groceryList) {
    List<GroceryItemResponseDTO> items =
        groceryList.getItems().stream().map(this::toItemDTO).toList();
