...
public class GroceryListController {
  ...
  public GroceryListDetailDTO getById(
      @Parameter(hidden = true) @RequestHeader("X-User-Id") UUID userId,
      @Parameter(description = "Identifier of the grocery list", required = true) @PathVariable
          UUID groceryListId) {
  ...
  @Operation(
      ...
      description =
          "Reads the selected recipes, asks the Gen AI service to deduplicate and sum their "
      ...
      responses = {
        ...
        @ApiResponse(
            responseCode = "422",
            description = "AI service returned no merged ingredients",
            content = @Content(schema = @Schema(implementation = Map.class))),
        @ApiResponse(
            responseCode = "502",
  ...
  @PatchMapping(
      value = "/{groceryListId}/items/{itemId}",
      consumes = MediaType.APPLICATION_JSON_VALUE,
      produces = MediaType.APPLICATION_JSON_VALUE)
  @Operation(
      summary = "Update a grocery item's purchased flag",
      description =
          "Marks a single item in the list as picked up (or not) without resending the whole list.",
      security = @SecurityRequirement(name = "bearerAuth"),
      responses = {
        @ApiResponse(responseCode = "200", description = "Item updated"),
        @ApiResponse(
            responseCode = "400",
            description = "Missing purchased flag",
            content = @Content(schema = @Schema(implementation = Map.class))),
        @ApiResponse(
            responseCode = "401",
            description = "Missing, expired, or invalid JWT",
  ...
  public GroceryItemResponseDTO updateItem(
      ...
      @Parameter(description = "Identifier of the grocery list", required = true) @PathVariable
          UUID groceryListId,
      @Parameter(description = "Identifier of the item", required = true) @PathVariable UUID itemId,
