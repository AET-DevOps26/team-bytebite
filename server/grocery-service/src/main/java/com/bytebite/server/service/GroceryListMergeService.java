...
/**
 * Merges several stored recipes into a new grocery list: reads the selected recipes from
 * the DB, delegates deduplication/summing to the gen-ai service, then persists the result
 * as a grocery list (linked back to its source recipes via grocery_list_recipes).
 */
@Service
public class GroceryListMergeService {
    ...
    public GroceryListMergeService(RecipeRepository recipeRepository,
    ...
    public GroceryListDetailDTO merge(UUID userId, MergeListRequest request) {
        ...
        for (UUID recipeId : recipeIds) {
            recipes.add(recipeRepository.findByIdAndUserId(recipeId, userId)
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                            "Recipe not found: " + recipeId)));
        }

        List<IngredientDTO> merged = callGenAiMerge(recipes,
                request.llmProvider() != null ? request.llmProvider() : DEFAULT_LLM_PROVIDER);
    ...
    private List<IngredientDTO> callGenAiMerge(List<Recipe> recipes, String llmProvider) {
        ...
        for (Recipe recipe : recipes) {
            List<IngredientDTO> ingredients = recipe.getItems().stream()
        ...
        if (response == null || response.ingredients() == null || response.ingredients().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY,
                    "AI service returned no merged ingredients");
        }
    ...
    private IngredientDTO toIngredientDTO(GroceryItem item) {
        ...
        return new IngredientDTO(item.getName(), quantity, item.getUnit(),
                item.getCategory().name(), false, null);
    }

    private List<GroceryItem> buildItems(List<IngredientDTO> ingredients, GroceryList groceryList) {
        List<GroceryItem> items = new ArrayList<>();
        for (IngredientDTO ingredient : ingredients) {
            GroceryItemRequestDTO dto = new GroceryItemRequestDTO(
                    ingredient.name(),
    ...
    /** Parses a free-form quantity string to a number, treating blanks/"N/A"/non-numeric as unspecified. */
    ...
    private GroceryListDetailDTO toDetail(GroceryList groceryList) {
        List<GroceryItemResponseDTO> items = groceryList.getItems().stream()
                .map(item -> new GroceryItemResponseDTO(
                        ...
                        item.isPurchased()))
                .toList();
        return new GroceryListDetailDTO(groceryList.getId(), groceryList.getName(),
                groceryList.getCreatedAt(), items);
    }
}
