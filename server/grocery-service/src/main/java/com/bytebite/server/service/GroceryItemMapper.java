...
/**
 * Builds {@link GroceryItem} entities from request payloads. Recipes and grocery lists share the
 * same item table, so they share the same field handling here; callers set the owning association
 * (and {@code purchased}) afterwards.
 */
final class GroceryItemMapper {
  ...
  /**
   * Creates an item with the common fields populated, validating name and normalizing
   * unit/category.
   */
  ...
  /**
   * Aliases mapping the gen-ai taxonomy (and common variants) onto the coarser {@link
   * GroceryCategory} enum. The gen-ai prompt is aligned to emit the enum tokens directly; this
   * table is defence-in-depth so legacy or drifting labels (e.g. "Dairy &amp; Eggs", "Dry Goods
   * &amp; Pasta", "Spices &amp; Herbs") still resolve to a real category instead of silently
   * collapsing to OTHER.
   */
  private static final Map<String, GroceryCategory> CATEGORY_ALIASES =
      Map.ofEntries(
